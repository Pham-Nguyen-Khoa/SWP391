...
for (let i = 1; i <= fishCount; i++) {
  ...
  card.className = "card";
  card.style =
    "box-shadow: 0px 1px 15px 4px rgb(0 0 0 / 30%); border-radius: 8px; border: none; margin-bottom: 100px; background: linear-gradient(#bdd2dc 0%, #93bfe4db 100%); padding-bottom: 20px;";
  card.innerHTML = `
...
document.querySelectorAll(".medication-select").forEach((selectElement) => {
  ...
  selectElement.addEventListener("change", function () {
    ...
    if (selectedMed) {
      ...
      const prescriptionTable = document
...
function addOrUpdateMedicationInTable(medication, prescriptionTable, tableId) {
  const existingRow = Array.from(prescriptionTable.rows).find(
    (row) => row.cells[1].textContent === medication.Name
  );
...
function saveFormData() {
  ...
  for (let i = 1; i <= fishCount; i++) {
    const prescriptionTable = document
      .getElementById(`prescriptionTable${i}`)
      .getElementsByTagName("tbody")[0];
...
function restoreFormData() {
  ...
  if (savedData) {
    ...
    data.forEach((prescription) => {
      const prescriptionTable = document
        .getElementById(`prescriptionTable${prescription.fish}`)
        .getElementsByTagName("tbody")[0];
...
if (payCenter) {
  ...
  payCenter.addEventListener("click", (e) => {
    ...
    document.getElementById("selectedMedicationsCenter").value =
      prescriptionData;
